...
class PSEELoader(object):
    ...
    def load_n_events(self, ev_count):
        ...
        if ev_count >= count:
            ...
            self.current_time = event_buffer[self._t_flag][ev_count - 1] + 1
        else:
            self._binary_format.stream_td_data(self._file, event_buffer, self._dtype, ev_count + 1)
            self.current_time = event_buffer[self._t_flag][ev_count]
    ...
    def load_delta_t(self, delta_t):
        ...
        while tmp_time < final_time and pos < self._end:
            ...
            tmp_time = buffer[self._t_flag][-1]
        ...
        idx = np.searchsorted(event_buffer[-1][self._t_flag], final_time)
    ...
    def seek_event(self, ev_count):
        ...
        if ev_count <= 0:
            ...
            self.current_time = np.fromfile(self._file, dtype=self._dtype, count=1)[self._t_flag][0] + 1
        else:
            ...
            self.current_time = np.fromfile(self._file, dtype=self._dtype, count=1)[self._t_flag][0]
    ...
    def seek_time(self, final_time, term_criterion=100000):
        ...
        while high - low > term_criterion:
            ...
            mid = np.fromfile(self._file, dtype=self._dtype, count=1)[self._t_flag][0]
        ...
        self.seek_event(low)
        final_buffer = np.fromfile(self._file, dtype=self._dtype, count=high - low)[self._t_flag]
    ...
    def total_time(self):
        ...
        time = np.fromfile(self._file, dtype=self._dtype, count=1)[self._t_flag][0]
        # restore the state
        self._file.seek(pos)
